...
const Home: React.FC = () => {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const updatedMetrics = {
        ...
        gusts: 15 + (seed % 20),
        visibility: 7 + (seed % 5),
        pressure: 1010 + (seed % 28),
        dewPoint: 3 + (seed % 8),
        uvIndex: seed % 3 === 0 ? 'Moderate' : 'Low',
        sunrise: '06:34',
        sunset: '17:32',
        highC: 12 + (seed % 8),
        lowC: 4 + (seed % 5),
        highF: 54 + (seed % 15),
        lowF: 39 + (seed % 10),
        cond: seed % 2 === 0 ? 'Drizzle' : 'Cloudy',
        emoji: seed % 2 === 0 ? '🌧️' : '☁️',
        summary: seed % 2 === 0 ? 'Cloudy conditions expected around 19:00.' : 'Clear intervals developing later.'
      };
  ...
  const handleThemeToggleAction = () => {
    ...
    triggerNotificationMessage('Application layout styling mode swapped.');
  };
  ...
  const handleFavoritesToggleAction = () => {
    ...
    if (favorites.includes(currentCity)) {
      ...
      triggerNotificationMessage(`${currentCity} profile removed from bookmarks list.`);
    } else {
      updatedRegister = [...favorites, currentCity];
      triggerNotificationMessage(`${currentCity} profile added to bookmarks list.`);
    }
